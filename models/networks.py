...
from .networks_block import Shift_Soft
...
def define_G(input_nc, output_nc, ngf, which_model_netG, opt, mask_global, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[], init_gain=0.02):
    ...
    elif which_model_netG == 'unet_shift_triple_soft':
        netG = UnetGeneratorShiftTriple_Soft(input_nc, output_nc, 8, opt, innerCos_list, shift_list, mask_global, \
                                                         ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    else:
...
class UnetSkipConnectionShiftTriple_Soft(nn.Module):
    def __init__(self, outer_nc, inner_nc, opt, innerCos_list, shift_list, mask_global, input_nc,
        ...
        super(UnetSkipConnectionShiftTriple_Soft, self).__init__()
        ...
        # soft shift
        shift = Shift_Soft(opt.threshold, opt.fixed_mask, opt.shift_sz, opt.stride, opt.mask_thred, opt.triple_weight)
        shift_list.append(shift)


        # TODO: Change these code according to soft attn.
